...
import com.base.ui.org.utils.BaseKeyboardUtil;
import com.base.ui.org.utils.BasePrexUtil;
import com.base.ui.org.view.edittext.BaseEditTextAutoZoom;
...
public class BaseTitle extends FrameLayout {
    ...
    private BaseEditTextAutoZoom mTvTitle;//Title的TextView控件
    ...
    private void initView(final Context context, AttributeSet attrs) {
        LayoutInflater.from(context).inflate(R.layout.include_base_title, this);

        mRootLayout = (RelativeLayout) findViewById(R.id.root_layout);
        mTvTitle = (BaseEditTextAutoZoom) findViewById(R.id.tv_rx_title);
        ...
        //获得这个控件对应的属性。
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.BaseTitle);
        ...
        try {
            ...
            mLeftIcon = a.getResourceId(R.styleable.BaseTitle_leftIcon, R.mipmap.previous_icon);//左边图标
            mRightIcon = a.getResourceId(R.styleable.BaseTitle_rightIcon, R.mipmap.set);//右边图标
            mLeftIconVisibility = a.getBoolean(R.styleable.BaseTitle_leftIconVisibility, true);//左边图标是否显示
        ...
        //******************************************************************************************以下属性初始化
        if (!BaseDataUtil.isNullString(mTitle)) {
            setTitle(mTitle);
        }
    ...
    private void initAutoFitEditText() {
        ...
        mTvTitle.setMaxHeight(BasePrexUtil.dip2px(55f));
        //don't forget to add min text size programmatically
        mTvTitle.setMinTextSize(37f);
        try {
            BaseEditTextAutoZoom.setNormalization((Activity) getContext(), mRootLayout, mTvTitle);
            BaseKeyboardUtil.hideSoftInput((Activity) getContext());
        } catch (Exception e) {
    ...
    public BaseEditTextAutoZoom getTvTitle() {
        return mTvTitle;
    }
