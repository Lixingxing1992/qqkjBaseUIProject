 ...
 * setTransparentStatusBar     : 设置透明状态栏(api大于19方可使用)
 ...
 hasNavagtion                :是否有虚拟键盘
 getNavagtionHeight          :获取虚拟键盘高度
 */
public class BaseBarUtil {
...
//      window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
     ...
     * @return
     */
    public static boolean hasNavagtion(Activity activity){
        WindowManager windowManager = activity.getWindowManager();

        DisplayMetrics displayMetricsReal = new DisplayMetrics();
        DisplayMetrics displayMetrics = new DisplayMetrics();

        windowManager.getDefaultDisplay().getRealMetrics(displayMetricsReal);
        windowManager.getDefaultDisplay().getMetrics(displayMetrics);

        // 真实高度
        int realHeight = displayMetricsReal.heightPixels;
        int height = displayMetrics.heightPixels;

        return realHeight - height > 0 ;
    }
     ...
     * @return
     */
    public static int getNavagtionHeight(Activity context){
        int result = 0;
        if(hasNavagtion(context)){
            int resourceId = context.getResources()
